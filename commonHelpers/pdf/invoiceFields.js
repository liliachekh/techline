const { PDFDocument, rgb } = require("pdf-lib")
 ...
 function staticInvoiceFields(height, fontSize, formatDate, order, customer){
  return [
    ...
    {
      type: "text",
      text: `Country, city, postal code: ${order.deliveryAddress.country} ${order.deliveryAddress.city} ${order.deliveryAddress.postal}`,
      x: 50,
...
function dynamicInvoiceFields (height, fontSize, order, customer, cartQuantity, verticalPosition) {
  ...
  {
    type: "text-dynamic",
    text: `${order.deliveryAddress.postal} ${order.deliveryAddress.city}`,
    x: 50,
